...
public class CamelCase {
	...
	public List<String> converterCamelCase(String stringTeste) {
		try {
	...
	private boolean isValidString(String stringTeste) {
	...
	//São considerados siglas todas as palavras que apresentarem todas as letras maiúsculas
	...
	private List<String> separarString(String tempString) {
		...
		tempString = tempString.replaceAll(			 //Adiciona um marcador (" ") entre...
				String.format("%s|%s|%s",
						"(?<=[A-Z])(?=[A-Z][a-z])",  //... uma letra maiúscula e outra maiúscula seguida de minúscula ("CPFContribuinte" => "CPF Contribuinte").
						"(?<=[^A-Z])(?=[A-Z])", 	 //... qualquer caracter não maiúscula e maiúscula ("nomeComposto" => "nome Composto", "10Primeiros" => "10 Primeiros", "NomeComposto" => "Nome Composto").
						"(?<=[A-Za-z])(?=[^A-Za-z])" //... qualquer letra (maiúscula ou minúscula) e qualquer não letra ("primeiros10" => "primeiros 10", "numero?" => "numero ?").
						)," ");
